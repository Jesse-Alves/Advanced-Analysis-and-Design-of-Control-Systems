...
tpu=25;     % Instante inicial da perturbaÁ„o da entrada
...
Apy=0;    % Amplitude da perturbaÁ„o da saÌda
...
u = 0; ud = 0;% => Para a aÁ„o Anti-Windup 
for k=1:kfinal
  ...
  elseif k>tpy/dT&& k<= (tpy+dy)/dT -1
   ...
   for l=1:m
    ...
    if abs(ud-u)>1e-5
     ...
     ud=Cc*xc+Dc*erro-kc*y;  % <====== Controle mudado para I+P
     %u=Cc*xc+Dc*erro;
     
    % ====> SATURA«√O acrescentada
    if ud>=0.8
        u = 0.8;    
    elseif ud<=0
        u = 0;        
